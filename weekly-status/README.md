...
- A delivered contribution is a `[TASK]` Issue whose current `state` is `closed`. Weekly delivery uses its `closed_at` within the reporting week. The report trusts the existing three-LGTM closure workflow; it does not require a review marker or check `state_reason` again. A reopened Issue does not count as delivered.
- The directory summary lists every immediate directory under `xiangshan-course/docs`, even with zero tasks or deliveries. It shows total tasks, weekly new tasks, weekly deliveries, current cumulative deliveries, current open tasks, and weekly delivery contributors. Each Issue belongs to one directory and counts once there; missing or invalid directory values are kept separately as `未分类`.
- Weekly delivered tasks are listed by directory with the seven template fields, identifying the task, assignees and commit references. Personal delivery credit goes to each current assignee, not the creator or closing bot. Without assignees, the delivery still counts for its directory and is marked as unattributed; no personal credit is guessed.
- Creation, assignment confirmation, review submission and LGTM are participation metrics only, not delivered contributions. Assignment and review submission use `github-actions[bot]` comments carrying `<!-- task-assignment:resolved claimant=... -->` and `<!-- task-review:pending executor=... -->` during the week.
...
- The other-task detail table includes current open tasks and closed tasks with any Issue creation, `updated_at`, `closed_at`, or comment activity in the week, excluding weekly deliveries already listed above. Its status column marks an open expired DDL as `（逾期）`.
...
Task status, assignees, DDL, directories, cumulative deliveries and contributor stock are current snapshots at report generation time, not historical reconstructions. Re-running a historical week does not restore historical state, and the report deliberately has no per-run timestamp, avoiding empty commits caused only by generation time. An open Issue with an old resolved marker is rendered as `进行中（已重新打开）`, never as completed. Only the latest `closed_at` is used; repeated closure cycles are not counted as separate deliveries. A closed Issue with no `closed_at` counts toward current cumulative deliveries but cannot be attributed to a reporting week. Bot marker edits, deletions, or markers emitted by accounts other than `github-actions[bot]` are intentionally not inferred; only the currently returned comment content is used for participation metrics.
