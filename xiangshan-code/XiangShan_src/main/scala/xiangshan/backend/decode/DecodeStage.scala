...
class DecodeStage(implicit p: Parameters) extends XSModule
  ...
  for ((decoder, i) <- decoders.zipWithIndex) {
    ...
    HISIONDebug( condition,"\n It's Decode %d:\n" +
                           ...
                           "--------------------------------------- \n"+
                           "                -srcType_0 = %d -lsrc_0 = %d \n" +
                           "                -srcType_1 = %d -lsrc_1 = %d \n" +
                           "                -selImm = 0x%x -imm = 0x%x \n" +
                           "                -rfWen = %d -ldest = %d \n" +
                           "--------------------------------------- \n" +
                           "", 
      i.U ,debugBits.instr, debugBits.debug.get.pc,
      debugBits.commitType,
      debugBits.fuType, debugBits.fuOpType,
      debugBits.srcType(0) ,debugBits.lsrc(0) ,
      debugBits.srcType(1) ,debugBits.lsrc(1) ,
      debugBits.selImm ,debugBits.imm ,
      debugBits.rfWen ,debugBits.ldest
    )
  }
