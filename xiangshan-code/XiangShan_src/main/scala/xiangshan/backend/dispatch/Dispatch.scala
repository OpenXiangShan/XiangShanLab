...
class Dispatch(implicit p: Parameters) extends XSModule with HasPerfEvents with HasVLSUParameters {
  ...
  val io = IO(new Bundle {
    // from rename
    val renameIn = Vec(RenameWidth, Flipped(ValidIO(new DecodeOutUop)))//组合逻辑
    ...
    for (i <- 0 until RenameWidth) {
      //goROB
      val special_pc = io.enqRob.req(i).bits.debug.get.pc
      val debugBits = io.enqRob.req(i).bits

      //这个时候就不能看输入了，因为如果看输入的话，这个时间是下一条指令进来的时间

      val special_condition = io.enqRob.req(i).valid
      val condition = special_condition && ( (p(DebugOptionsKey).DebugAll).B || special_pc === (p(DebugOptionsKey).DebugPC).U)
      val debugPdest = Mux(debugBits.rfWen.asBool, debugBits.pdest, 0.U)
      HISIONDebug( condition ,"\n In Dispatch toROB %d:\n" +
                             "                -instr = 0x%x -pc = 0x%x  \n" +
                             "                -commitType = 0x%x -fuType = 0x%x -fuOpType = 0x%x \n" +
                             "---------------------------------------  \n" +
                             "                -srcType_0 = %d -psrc_0 = %d  \n" +
                             "                -srcType_1 = %d -psrc_1 = %d  \n" +
                             "                -selImm = 0x%x -imm = 0x%x \n" +
                             "                -rfWen = %d -pdest = %d  \n" +
                             "---------------------------------------  \n" +
                             "                -roIdx = %d  \n" +
                             "                -numUops = %d  \n" +
                             ...
                             "--------------------------------------- \n" 
                             , 
        i.U ,debugBits.debug.get.instr, debugBits.debug.get.pc,
        debugBits.commitType,debugBits.fuType, debugBits.fuOpType,
        debugBits.srcType(0) ,debugBits.psrc(0) ,
        debugBits.srcType(1) ,debugBits.psrc(1) ,
        debugBits.selImm ,debugBits.imm ,
        debugBits.rfWen  ,debugPdest,
        debugBits.robIdx.value,
        debugBits.numUops,
        debugBits.numWB
      )
  ...
  for (i <- 0 until LSQEnqWidth) {
      //toLSQ
      val special_pc = io.toMem.lsqEnqIO.req(i).bits.pc
      val debugBits = io.toMem.lsqEnqIO.req(i).bits
  
      //这个时候就不能看输入了，因为如果看输入的话，这个时间是下一条指令进来的时间
      val special_condition = io.toMem.lsqEnqIO.req(i).valid && 
                              (io.toMem.lsqEnqIO.needAlloc(i) =/= 0.U) &&
                              (io.toMem.lsqEnqIO.canAccept)
      ...
      HISIONDebug( condition ,"\n In Dispatch toLSQ %d: \n" +
                             ...
                             "--------------------------------------- \n" 
                             , 
        i.U ,debugBits.instr, debugBits.pc,
        debugBits.commitType,debugBits.fuType, debugBits.fuOpType,
        debugBits.srcType(0) ,debugBits.psrc(0) ,
        debugBits.srcType(1) ,debugBits.psrc(1) ,
        debugBits.selImm ,debugBits.imm ,
        ...
        debugBits.numUops,
        debugBits.robIdx.value,
        debugBits.sqIdx.value
      )

  }

  for (i <- 0 until IQEnqSum) {
      //toIQ
      val special_pc = io.toIssueQueues(i).bits.debug.get.pc
      val debugBits = io.toIssueQueues(i).bits
  
      //这个时候就不能看输入了，因为如果看输入的话，这个时间是下一条指令进来的时间
      val special_condition = io.toIssueQueues(i).valid && io.toIssueQueues(i).ready 

      val condition = special_condition && ( (p(DebugOptionsKey).DebugAll).B || special_pc === (p(DebugOptionsKey).DebugPC).U)

      val debugPdest =  debugBits.pdest
      HISIONDebug( condition ,"\n In Dispatch to IssueQueues %d: \n" +
                             "                -pc = 0x%x  \n" +
                             "                -fuType = 0x%x -fuOpType = 0x%x \n" +
                             "--------------------------------------- \n" +
                             "                -srcType_0 = %d -psrc_0 = %d \n" +
                             "                -srcType_1 = %d -psrc_1 = %d \n" +
                             "                -selImm = 0x%x -imm = 0x%x \n" +
                             "                -rfWen = %d -pdest = %d\n" +
                             "                -srcState_0 = %d -srcState_1 = %d\n" +
                             ...
                             , 
        i.U ,special_pc,
        debugBits.fuType, debugBits.fuOpType,
        debugBits.srcType(0) ,debugBits.psrc(0) ,
        debugBits.srcType(1) ,debugBits.psrc(1) ,
        debugBits.selImm ,debugBits.imm ,
        debugBits.rfWen  ,debugPdest,

        debugBits.srcState(0),debugBits.srcState(1),
...
for (i <- 0 until IQEnqSum) {
  ...
  val queueIndex = i / 2
  HISIONDebug( condition && (queueIndex.U === 0.U ), " \n It will be sent to issueQueueALU0BJU0_AluCsrFenceBrhJmp\n") 
  HISIONDebug( condition && (queueIndex.U === 1.U ), " \n It will be sent to issueQueueALU1BJU1_AluDivBrhJmp\n") 
  HISIONDebug( condition && (queueIndex.U === 2.U ), " \n It will be sent to issueQueueALU2BJU2_AluI2FVsetriwiVsetriwvfl2vBrhJmp\n") 
  HISIONDebug( condition && (queueIndex.U === 3.U ), " \n It will be sent to issueQueueALU3_AluBku\n") 
  HISIONDebug( condition && (queueIndex.U === 4.U ), " \n It will be sent to issueQueueALU4_AluMul\n") 
  HISIONDebug( condition && (queueIndex.U === 5.U ), " \n It will be sent to issueQueueALU5_AluMul\n") 
  HISIONDebug( condition && (queueIndex.U === 6.U ), " \n It will be sent to issueQueueLDU0_Ldu\n") 
  HISIONDebug( condition && (queueIndex.U === 7.U ), " \n It will be sent to issueQueueLDU1_Ldu\n") 
  HISIONDebug( condition && (queueIndex.U === 8.U ), " \n It will be sent to issueQueueLDU2_Ldu\n") 
  HISIONDebug( condition && (queueIndex.U === 9.U ), " \n It will be sent to issueQueueSTA0_StaMou&QueueSTD0_StdMoud\n") 
  HISIONDebug( condition && (queueIndex.U === 10.U), " \n It will be sent to issueissueQueueSTA1_StaMou&issueQueueSTD1_StdMoud\n") 
  HISIONDebug( condition && (queueIndex.U === 11.U), " \n It will be sent to issueQueueFEX0_FaluFmacFcvtFcmpF2v\n") 
  HISIONDebug( condition && (queueIndex.U === 12.U), " \n It will be sent to issueQueueFEX1_FaluFmacFdiv\n") 
  HISIONDebug( condition && (queueIndex.U === 13.U), " \n It will be sent to issueQueueFEX2_FaluFmacFdiv\n") 
  HISIONDebug( condition && (queueIndex.U === 14.U), " \n It will be sent to issueQueueFEX3_FaluFmac\n") 
  HISIONDebug( condition && (queueIndex.U === 15.U), " \n It will be sent to issueQueueVFEX0_VialuFixFaluFvmaVimacVppuVipuVFcvtVsetrvfFvMove\n") 
  HISIONDebug( condition && (queueIndex.U === 16.U), " \n It will be sent to issueQueueVFEX1_VialuFixFaluFvmaVfdivVvid\n") 
  HISIONDebug( condition && (queueIndex.U === 17.U), " \n It will be sent to issueQueueVLSU0_VlduVstuVseglduVsegstu\n") 
  HISIONDebug( condition && (queueIndex.U === 18.U), " \n It will be sent to issueQueueVLSU1_VlduVstu\n") 
