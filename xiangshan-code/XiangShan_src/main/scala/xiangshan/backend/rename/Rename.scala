...
class Rename(implicit p: Parameters) extends XSModule with HasCircularQueuePtrHelper with HasPerfEvents {
  ...
  for (i <- 0 until RenameWidth) {
    ...
    val special_pc = io.out(i).bits.debug.get.pc
    val special_condition = io.out(i).valid  &&  io.out(i).ready
    val condition = special_condition && ( (p(DebugOptionsKey).DebugAll).B || special_pc === (p(DebugOptionsKey).DebugPC).U)
    val debugBits = io.out(i).bits
    ...
    HISIONDebug( condition ,"\n In Rename %d: \n" +
      ...
      debugBits.rfWen, debugBits.ldest ,debugPdest,
