...
//    io.sqCancelCnt := redirectCancelCount
  ...
  for (i <- 0 until StoreQueueSize) {
      //toSQ
    val special_pc = dataEntries(i).debugUop.get.pc
    val debugBits = dataEntries(i)
  
    val special_condition = ctrlEntries(i).allocated
    ...
    HISIONDebug( condition ,"\n StoreQ %d status :\n" +
                           "                -pc = 0x%x -robIdx = %d \n" +
                           "--------------------------------------- \n" +
                           "                -VaddrVaid = %d -DataVaid = %d\n" +
                           "                -Vaddr = 0x%x -Data = 0x%x \n" +
                           "--------------------------------------- \n" +
                           "                -SQcommit = %d  \n" +
                           "--------------------------------------- \n" 
                           , 
      i.U ,special_pc, debugBits.uop.robIdx.value,
      ctrlEntries(i).addrValid, ctrlEntries(i).dataValid,
      debugBits.vaddr ,debugBits.data ,
      ctrlEntries(i).committed
    )
