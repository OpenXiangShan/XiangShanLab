# Commit Log
- Issue: #5292
- Issue URL: https://github.com/OpenXiangShan/XiangShan/pull/5292
- Issue state: closed
- Tested RTL commit: -
- Related PR: #5292
- PR URL: https://github.com/OpenXiangShan/XiangShan/pull/5292
- Changed files: 4
- Additions: 14
- Deletions: 34
...
index 854f4560435..3b080d28ea3 100644
...
@@ -64,12 +64,6 @@ class LqExceptionBuffer(implicit p: Parameters) extends XSModule with HasCircula
...
@@ -81,8 +75,7 @@ class LqExceptionBuffer(implicit p: Parameters) extends XSModule with HasCircula
         res(i).bits := bits(i)
       }
...
@@ -92,16 +85,11 @@ class LqExceptionBuffer(implicit p: Parameters) extends XSModule with HasCircula
     }
   }
...
index ca4ff7e5959..985116977d8 100644
...
-
   def selectOldest[T <: LsPipelineBundle](valid: Seq[Bool], bits: Seq[T]): (Seq[Bool], Seq[T]) = {
     assert(valid.length == bits.length)
     if (valid.length == 0 || valid.length == 1) {
@@ -122,8 +116,7 @@ class StoreExceptionBuffer(implicit p: Parameters) extends XSModule with HasCirc
         res(i).bits := bits(i)
       }
       val oldest = Mux(valid(0) && valid(1),
-        Mux(isAfter(bits(0).uop.robIdx, bits(1).uop.robIdx) ||
...
@@ -133,16 +126,11 @@ class StoreExceptionBuffer(implicit p: Parameters) extends XSModule with HasCirc
     }
   }
...
index 060c3518376..dfe8a2c8ca0 100644
--- a/src/main/scala/xiangshan/mem/vector/VMergeBuffer.scala
+++ b/src/main/scala/xiangshan/mem/vector/VMergeBuffer.scala
@@ -108,6 +108,8 @@ abstract class BaseVMergeBuffer(isVStore: Boolean=false)(implicit p: Parameters)
...
index a12afef06c0..ddf13fc2aec 100644
