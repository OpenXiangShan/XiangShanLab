...
|---|---|
| **指令名称** | `cbo.zero rs1` |
| **编码格式** | `000000000100_rs1[4:0]_010_00000_0001111`，32 位；`imm[11:0]=000000000100`、`funct3=010`、`rd=00000`、`opcode=0001111`；`rs1` 为地址源；本地内部操作编码 `LSUOpType.cbo_zero=0b0111` 不属于上述指令位字段 |
...
**语义边界。** `cbo.zero` 请求以 `rs1` 给出的地址定位一个 cache block，并将该块的全部字节写为零；它不产生 GPR 写回，但会产生实际的内存写入，因此不能视为只改变缓存元数据的 clean/flush/inval。当前本地实现对 cacheable 路径和 MMIO/uncache 属性分别处理，具体写零动作由 StoreQueue 的 `cboZero` 路径完成。当前本地实现把它作为 `FuType.stu` 处理，并在 StoreQueue 中等到 ROB 头部、写缓冲排空后才向 DCache CMO 接口发请求。
