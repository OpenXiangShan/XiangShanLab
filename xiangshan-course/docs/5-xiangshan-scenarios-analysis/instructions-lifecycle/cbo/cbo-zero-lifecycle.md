...
|---|---|
| **指令名称** | `cbo.zero rs1` |
| **编码格式** | CBO 指令使用 `opcode=0001111`、`funct3=010`、`rs1` 地址源和 `rd=00000`；`cbo.zero` 的本地 CMO 子操作编码为 `LSUOpType.cbo_zero = 0b0111`。具体指令匹配常量见 [DecodeUnit.scala][D] 与 [package.scala][OP] |
| **RISC-V 扩展** | `Zicbom`；本地构建还由 `HasCMO` 和 CSR 的 `cboCF` 能力控制 |
| **是否有压缩格式** | 无 C 扩展压缩编码；CBO 指令本身是 32 位指令，前端仍可从启用 C 扩展的半字窗口中取出 |
| **指令分类** | 缓存块管理 / CMO / zero |
| **FuType** | `FuType.stu` |
| **FuOpType** | `LSUOpType.cbo_zero` |
| **目标 FU** | Store 地址路径 → StoreQueue CMO 状态机 → DCache `StoreQueue zero path` → TileLink CBO 请求 |
| **分析日期** | 2026-09-06 |

**语义边界。** `cbo.zero` 请求以 `rs1` 给出的地址定位一个 cache block，并将该块的全部字节写为零；它不产生 GPR 写回，但会产生实际的内存写入，因此不能视为只改变缓存元数据的 clean/flush/inval。当前本地实现对 cacheable 路径和 MMIO/uncache 属性分别处理，具体写零动作由 StoreQueue 的 `cboZero` 路径完成。当前本地实现把它作为 `FuType.stu` 处理，并在 StoreQueue 中等到 ROB 头部、写缓冲排空后才向 DCache CMO 接口发请求。本文以本地 `/nfs/home/wanghao/emuByYuan/stable-kmh-v2` 源码为唯一实现依据；没有把现有报告中的截图周期当作本次源码已经证明的实测值。
...
| 项目 | 内容 |
|---|---|
| **brTable 匹配** | `cbo.zero` 是非控制流指令，预译码表默认给出 `BrType.notCFI`；不产生分支预测目标 |
...
| **关键译码结果** | `fuType=FuType.stu`、`fuOpType=LSUOpType.cbo_zero`、`srcType(0)=reg`、第二源为 `DC`、`selImm=IMM_S`、无整数写回；表项位于 [DecodeUnit.scala][D] 478–481 行 |
...
| **源操作数** | 一个整数地址源 `rs1`；立即数由 `IMM_S` 解码，但 CBO zero 的 cache-block 地址最终由访存路径使用 |
...
| **LSQ 分配** | CBO 作为 `FuType.stu` 进入 StoreQueue 的记录和地址有效流程；`cbo.zero` 在 ROB 头部通过专用 zero-to-SBuffer 逻辑产生全零写入，不等同于普通 Store 的单次数据请求 |
| **特殊顺序要求** | StoreQueue 在 `deqCanDoCbo` 条件下才开始 zero 写入；该条件要求 ROB 头、地址有效、已分配且无异常，并由 `memBackTypeMM` 过滤 |
...
| 项目 | 内容 |
|---|---|
| **执行单元** | Store/Hybrid 地址执行 → StoreQueue CMO 状态机 → DCache `StoreQueue zero path` |
...
| **FSM 状态机** | StoreQueue 的 `s_idle/s_req/s_resp/s_wb/s_wait` 五状态；`cbo.zero` 另有 `cboZeroValid/cboZeroWaitFlushSb` 控制，不走普通 CMOUnit 四状态 |
| **关键输出信号** | `flushSbuffer.valid`、`cboZeroToSb`、`cboZeroFlushSb`、`cboZeroValid`、`cboZeroWaitFlushSb`、`cboZeroStout.valid`、`uop.flushPipe` |
...
| `s_req` | 等待写缓冲排空或发送 zero 写入 | 未排空时 `flushSbuffer.valid=1`；排空且 `cboZeroStout.fire` 后发送 CMO | `io.flushSbuffer.empty` → `cboFlushedSb=1`；`cboZeroStout.fire` → `s_resp` |
| `s_resp` | 普通 CMO 响应状态；`cbo.zero` 不使用该状态完成 zero 写入 | 不承载本文目标指令的主要完成信号 | 本条指令的 zero 路径不依赖该状态 |
| `s_wb` | CMO 已完成，等待向 ROB 返回执行结果 | `mmioStout.valid=1`，并将 `uop.flushPipe := deqCanDoCbo` | `mmioStout.fire`：有异常 → `s_idle`，否则 → `s_wait` |
| `s_wait` | 等待 ROB 提交 | 无新的 zero 写入 | `scommit>0` → `s_idle` |

`cbo.zero` 不使用 DCache `CMOUnit` 的 `CMOReq` 事务。StoreQueue 在 `cboZeroToSb` 检测到 zero store 已进入 SBuffer 后，置位 `cboZeroValid` 和 `cboZeroWaitFlushSb`；等待 SBuffer 排空后，通过 `cboZeroStout` 向 MemBlock 返回完成结果。源码明确要求同一时刻最多一个 zero-to-SBuffer 事件，且 zero 的写入粒度按 cache line 中的 XLEN 单元推进。它不是把 `CMOReq.opcode=3` 发送给 DCache CMOUnit 的替代写法。
...
| **写回延迟** | 从 `cboZeroStout.fire` 到 `mmioStout.fire` 还要经过 `s_wb` 的 ready；若写回端口被阻塞则可变 |
...
| **是否触发 flush** | `cbo.zero` 的完成 uop 设置 `flushPipe`，源码注释说明这是为了保持 CMO 顺序；这不是分支错误 redirect，而是提交前的顺序化流水线 flush 请求 |
| **是否触发 redirect** | 正常 zero 不产生分支目标 redirect；`flushPipe` 可能触发 ROB 的通用 flush/恢复控制，具体是否形成前端 redirect 取决于 ROB 当前状态 |
...
```mermaid
flowchart TD
  IFU[IFU / IBuffer] --> Decode[Decode: stu / cbo_zero]
  ...
  SB --> SQ
  SQ -->|zero store beats| SB
  SB -->|zero completion| SQ
...
| 信号名 | 方向 | 位宽 | 语义 | 持续方式 |
|---|---|---|---|---|
| `cboZeroStout.valid/ready` | StoreQueue → MemBlock | 各 1 | zero store 完成结果写回握手 | Decoupled，`fire=valid&&ready` |
| `zero store data` | StoreQueue → DCache | 3 | `0=clean`、`1=flush`、`2=inval`、`3=zero` | 请求握手时采纳 |
| `cboZeroStout.bits.uop.sqIdx` | StoreQueue → MemBlock | SQ index 宽度 | 标识完成的 zero store 记录 | 随完成结果保持到 fire |
| `io.flushSbuffer.valid/empty` | StoreQueue ↔ MemBlock | 各 1 | zero 写入前后的 SBuffer 排空控制 | 请求为电平，empty 为状态 |
...
| `cboZeroValid/cboZeroWaitFlushSb` | StoreQueue 内部 | 1/1 | 标记 zero 已产生、等待 SBuffer 排空 | 状态寄存器电平 |
...
| zero write | 可变 | `T_cmo_req` | 等 DCache zero write path ready |
| SBuffer completion | 可变 | `T_cmo_resp` | TileLink D 通道 zero completion 返回；下层延迟不由本模块限定 |
...
公式用于分段记录事件间隔。`T_cmo_req` 不等于 A 通道发送拍，`T_cmo_resp` 也不等于 D 通道收到拍；两者要分别记录 `cboZeroStout.fire`、`resp_chanD.fire`、`cboZeroStout.fire`。若 StoreQueue 因异常在 `s_wb` 直接回收，则成功路径公式不适用。

### 4.3 最佳/最差情况

| 场景 | 周期数 | 条件 |
|---|---|---|
| **最佳** | 仍为可变，至少包含 StoreQueue/DCache FSM 状态推进 | 指令已经到 ROB 头、地址有效、SBuffer 已空、zero path and SBuffer are ready、写回 ready |
| **典型** | 无匹配波形，不能给绝对值 | 可能经历一个或多个 StoreQueue/SBuffer 等待周期和多个 zero store beat 写入 |
...
{ "signal": [
  ...
  { "name": "zero write", "wave": "0...1...........0" },
  { "name": "zero completion/response", "wave": "0........1......0" },
...
| 异常类型 | 检测位置 | 检测条件 | 处理方式 |
|---|---|---|---|
| 非法指令 | DecodeUnit | `!HasCMO` 或 CSR `illegalInst.cboCF` | 设置 `illegalInstr`，不进入有效 zero 写入 |
...
| CMO 被下层拒绝 | Store/Hybrid/StoreQueue | 地址或权限异常 | `storeAccessFault`，在 `mmioStout` 写回异常 |
| CMO 响应损坏 | StoreQueue/下层写入路径 | 下层响应错误 | `hardwareError`，不作为成功 zero 提交 |
| SBuffer 背压 | StoreQueue/SBuffer | zero 写入尚未完成或 SBuffer 未空 | 保持 `cboZeroWaitFlushSb`，直到写回条件满足 |

**边界分析。** CBO zero 操作按 cache block 地址发送，而不是把一个跨边界请求拆成多个普通数据访问：

| 边界场景 | 源码可证明的行为 |
|---|---|
| 页内、cache line 内的对齐地址 | Store/Hybrid 完成翻译与权限后，StoreQueue 对物理地址执行 `get_block_addr`，向 SBuffer/下层写入 zero store 数据 |
| 页边界附近地址 | 地址翻译和权限检查先于 zero 写入；本模块没有把一条 CBO zero 拆成两个页请求。若地址本身不合法，异常阻止 `deqCanDoCbo` |
| Cache line 边界附近地址 | `CMOReq.address` 是一个 block 地址，DCache zero offset 按 cache line 大小推进；源码没有跨两个 block 的合并语义 |
...
| 触发条件 | 类型 | 影响范围 | 恢复方式 |
|---|---|---|---|
| 更老分支/异常在 CMO 发出前触发 redirect | 通用 redirect/kill | 未到 ROB 头的 CBO 及年轻指令 | 按 ROB/LSQ redirect 规则取消，不能产生 zero 写入 |
...
| TileLink A/D 通道背压 | CMO FSM stall | StoreQueue 当前 zero 事务 | StoreQueue zero 控制保持，直到 SBuffer empty 和 `cboZeroStout.fire` |
...
| 交互场景 | 影响指令 | 行为 |
|---|---|---|
| 更老 Store 尚在 SBuffer | `cbo.zero` | 先请求 flush，等 `io.flushSbuffer.empty` 后再发 CMO |
| 更年轻 Load/Store | 年轻内存指令 | CMO 的 `flushPipe` 和 ROB 头规则防止其越过 CMO 观察到错误顺序 |
| `cbo.flush` | 同一 CMO 路径 | flush 写回/失效缓存块；zero 实际写入全零，二者不能等同 |
| `cbo.inval` | 同一 CMO 路径 | inval 使缓存副本失效；zero 修改块内容，二者不能混用 |
| `cbo.zero` | StoreQueue 特殊路径 | 可能产生多次写回／zero 操作，不能与 zero 的一次 CMOReq 状态机等同 |
| 普通 MMIO store | StoreQueue uncache 路径 | 普通 MMIO 使用 `M_XWR`；CBO zero 显式禁用该普通 uncache 请求并走 zero-to-SBuffer |
| Fence/其他 flush-SB 操作 | MemBlock | MemBlock 断言不同 flush 请求不应同时有效；zero 与 fence 的 SBuffer 排空资源共享 |
...
| CMO 事务窗口 | `cboZeroStout.fire` | `cboZeroStout.fire` / 异常 | 可变 | 一旦下层接受请求，不能当作普通可撤销 ALU 操作；实现以顺序化和完成响应约束 |
...
| Cache block 是否脏、下层响应时延 | Cache/互连时序 | zero 路径统一等待 CMO 响应；脏/干块和一致性状态仍可能造成可观测差异 |
...
| 保证 | 机制 | 代码依据 |
|---|---|---|
| 更老 store 先于 zero 对外可见 | zero 写入前等待 `flushSbuffer.empty` | [StoreQueue.scala][SQ] 1031–1035 |
| zero 只能在 ROB 头执行 | `deqCanDoCbo` 要求 pending ROB 项成为当前头部且无异常 | [StoreQueue.scala][SQ] 841–847、983–985 |
| zero 写入与普通 uncache 请求不混用 | `deqCanDoCbo` 时禁用 `io.uncache.req.valid` | [StoreQueue.scala][SQ] 1006–1025 |
| 下层完成后才向 ROB 写回 | `cboZeroStout.fire → ROB 完成 → commit` | [StoreQueue.scala][SQ] 1015–1019、1053–1058 |
| zero 与 flush/inval 语义分离 | CMO opcode 分别编码为 0/1/2 | [DCacheWrapper.scala][CW] 619–627 |
...
| 指标 | 值 | 说明 |
|---|---|---|
| **执行吞吐** | 单个 StoreQueue CMO FSM 一次处理一条事务 | `cboZeroValid` 和 `cboZeroWaitFlushSb` 控制 zero store 的产生与 SBuffer 排空；不能从 MissQueue 条目数推导每拍多条 zero |
| **执行延迟** | `UncertainLatency`，由 ROB 头等待、drain、A/D 往返和写回决定 | 不存在源码证明的固定周期 |
| **端口占用** | Store/STA 地址路径、StoreQueue zero-to-SBuffer 路径、SBuffer 写入和 Store writeback | 不使用普通 DCache CMOUnit 的 TileLink CMO 端口 |
| **流水线阻塞** | ROB 头顺序化、SBuffer drain、zero store beat 写入、`cboZeroStout.ready` | 完成 uop 带 `flushPipe`，会进一步影响年轻指令恢复 |
...
| `HasCMO` | 配置项 | 未启用时 zero/flush/inval 进入非法指令路径 | [DecodeUnit.scala][EX] 913–915 行 |
| `CacheLineSize` / `XLEN` | 由缓存与核心参数决定 | 决定 zero store beat 数和地址推进步长 | [StoreQueue.scala][SQ] 837 行附近 |
| `PAddrBits` | SoC 参数推导 | CMO physical block address 位宽 | [DCacheWrapper.scala][CW] 619–627 行 |
| `cache_error_enable` | CSR/配置控制 | 该 CBO 路径是否将下层错误显式转异常，依本地 CMOResp 接口处理 | StoreUnit/StoreQueue 的异常路径 |
...
| zero 写入/响应 Bundle | [cache/dcache/DCacheWrapper.scala][CW] | 619–627、851–852 |
