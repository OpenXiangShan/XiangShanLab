...
|---|---|
| **指令名称** | `cbo.flush rs1` |
| **编码格式** | CBO 指令使用 `opcode=0001111`、`funct3=010`、`rs1` 地址源和 `rd=00000`；`cbo.flush` 的本地 CMO 子操作编码为 `LSUOpType.cbo_flush = 0b1101`。具体指令匹配常量见 [DecodeUnit.scala][D] 与 [package.scala][OP] |
| **RISC-V 扩展** | `Zicbom`；本地构建还由 `HasCMO` 和 CSR 的 `cboCF` 能力控制 |
| **是否有压缩格式** | 无 C 扩展压缩编码；CBO 指令本身是 32 位指令，前端仍可从启用 C 扩展的半字窗口中取出 |
| **指令分类** | 缓存块管理 / CMO / flush |
| **FuType** | `FuType.stu` |
| **FuOpType** | `LSUOpType.cbo_flush` |
| **目标 FU** | Store 地址路径 → StoreQueue CMO 状态机 → DCache `CMOUnit` → TileLink CBO 请求 |
| **分析日期** | 2026-09-06 |

**语义边界。** `cbo.flush` 请求以 `rs1` 给出的地址定位一个 DCache cache block，将该块按 CMO flush 操作写回并从当前缓存层移除；它不产生 GPR 写回，也不是普通 store。具体下层处理由 `CMOReq.opcode=1` 和一致性协议实现。当前本地实现把它作为 `FuType.stu` 处理，并在 StoreQueue 中等到 ROB 头部、写缓冲排空后才向 DCache CMO 接口发请求。本文以本地 `/nfs/home/wanghao/emuByYuan/stable-kmh-v2` 源码为唯一实现依据；没有把现有报告中的截图周期当作本次源码已经证明的实测值。
...
| 项目 | 内容 |
|---|---|
| **brTable 匹配** | `cbo.flush` 是非控制流指令，预译码表默认给出 `BrType.notCFI`；不产生分支预测目标 |
...
| **关键译码结果** | `fuType=FuType.stu`、`fuOpType=LSUOpType.cbo_flush`、`srcType(0)=reg`、第二源为 `DC`、`selImm=IMM_S`、无整数写回；表项位于 [DecodeUnit.scala][D] 478–481 行 |
...
| **源操作数** | 一个整数地址源 `rs1`；立即数由 `IMM_S` 解码，但 CBO flush 的 cache-block 地址最终由访存路径使用 |
...
`cbo.flush` 的 `CMOReq.opcode=1`，地址由 StoreQueue 的 `get_block_addr(paddr)` 得到；DCache CMOUnit 使用 `blockBytes` 对齐大小创建 CacheBlockOperation。该路径不是普通 `M_XWR` uncache store：StoreQueue 先关闭普通 uncache 请求，再使用 `cmoOpReq`。
...
| **是否触发 flush** | `cbo.flush` 的完成 uop 设置 `flushPipe`，源码注释说明这是为了保持 CMO 顺序；这不是分支错误 redirect，而是提交前的顺序化流水线 flush 请求 |
| **是否触发 redirect** | 正常 flush 不产生分支目标 redirect；`flushPipe` 可能触发 ROB 的通用 flush/恢复控制，具体是否形成前端 redirect 取决于 ROB 当前状态 |
...
```mermaid
flowchart TD
  IFU[IFU / IBuffer] --> Decode[Decode: stu / cbo_flush]
  ...
  SB --> SQ
  SQ -->|CMOReq opcode=flush, block address| CMO[ DCache MissQueue CMOUnit]
...
| CMO 响应损坏 | CMOUnit/StoreQueue | `CMOResp.corrupt && !denied` | `hardwareError`，不作为成功 flush 提交 |
| WFI | CMOUnit | `wfiReq` 有效 | 禁止 A 通道 CMO 请求，报告 `wfiSafe` 条件 |

**边界分析。** CBO flush 操作按 cache block 地址发送，而不是把一个跨边界请求拆成多个普通数据访问：

| 边界场景 | 源码可证明的行为 |
|---|---|
| 页内、cache line 内的对齐地址 | Store/Hybrid 完成翻译与权限后，StoreQueue 对物理地址执行 `get_block_addr`，向 CMOUnit 发一次 block operation |
| 页边界附近地址 | 地址翻译和权限检查先于 CMO 请求；本模块没有把一条 CBO flush 拆成两个页请求。若地址本身不合法，异常阻止 `deqCanDoCbo` |
...
| 交互场景 | 影响指令 | 行为 |
|---|---|---|
| 更老 Store 尚在 SBuffer | `cbo.flush` | 先请求 flush，等 `io.flushSbuffer.empty` 后再发 CMO |
| 更年轻 Load/Store | 年轻内存指令 | CMO 的 `flushPipe` 和 ROB 头规则防止其越过 CMO 观察到错误顺序 |
| `cbo.clean` | 同一 CMO 路径 | opcode 不同；clean 写回但保留缓存块，不能把 clean 的保留语义套给 flush |
| `cbo.inval` | 同一 CMO 路径 | opcode 不同；inval 的一致性/数据保留语义不同，本文只分析 flush |
| `cbo.zero` | StoreQueue 特殊路径 | 可能产生多次写回／zero 操作，不能与 flush 的一次 CMOReq 状态机等同 |
| 普通 MMIO store | StoreQueue uncache 路径 | 普通 MMIO 使用 `M_XWR`；CBO flush 显式禁用该普通 uncache 请求并走 `cmoOpReq` |
...
| Cache block 是否脏、下层响应时延 | Cache/互连时序 | flush 路径统一等待 CMO 响应；脏/干块和一致性状态仍可能造成可观测差异 |
...
| 保证 | 机制 | 代码依据 |
|---|---|---|
| 更老 store 先于 flush 对外可见 | CMO 请求前等待 `flushSbuffer.empty` | [StoreQueue.scala][SQ] 1031–1035 |
| flush 只能在 ROB 头执行 | `deqCanDoCbo` 要求 pending ROB 项成为当前头部且无异常 | [StoreQueue.scala][SQ] 841–847、983–985 |
...
| flush 与 flush/inval 语义分离 | CMO opcode 分别编码为 0/1/2 | [DCacheWrapper.scala][CW] 619–627 |
...
| 指标 | 值 | 说明 |
|---|---|---|
| **执行吞吐** | 单个 StoreQueue CMO FSM 一次处理一条事务 | `CMOUnit.req.ready` 仅在 idle；不能从 MissQueue 条目数推导每拍多条 flush |
...
| `HasCMO` | 配置项 | 未启用时 flush/flush/inval/zero 进入非法指令路径 | [DecodeUnit.scala][EX] 913–915 行 |
