...
| 阶段 | 延迟 | 关键行为 | 代码位置 |
|---|---|---|---|
| ICache/MainPipe | 可变 | 普通取指获取包含 `hfence.vvma` 的指令块；该指令不是控制流指令 | [IFU.scala][IF]、[ICache.scala][IC] |
...
| 项目 | 内容 |
|---|---|
| **brTable 匹配** | `hfence.vvma` 不是分支、跳转、调用或返回指令，正常情况下不匹配控制流预测条目，`brType` 为非控制流类型 |
| **PreDecodeInfo 字段** | `valid=1`、`isRVC=0`、`brType=notCFI`，不产生 call/return 属性；具体字段由 [PreDecode.scala][PD] 定义 |
| **是否有专用检测逻辑** | 无前端专用 HFENCE 预译码逻辑；后端 `DecodeUnit` 识别 `HFENCE.VVMA` |
...
| 项目 | 内容 |
|---|---|
| **是否触发 remaskFault** | `hfence.vvma` 自身不满足控制流指令条件，因此不主动触发；同一窗口内其他指令仍可能触发 |
...
| **关键译码结果** | `fuType=FuType.fence`、`fuOpType=FenceOpType.hfence_v`、两个源操作数为 `SrcType.reg`、目标为 `SrcType.X`、`noSpec=true`、`blockBack=true`、`flushPipe=true` |
| **代码位置** | [DecodeUnit.scala][D] 490 行附近 |

```scala
HFENCE.VVMA -> XSDecode(
  SrcType.reg, SrcType.reg, SrcType.X,
  FuType.fence, FenceOpType.hfence_v, SelImm.X,
  noSpec = T, blockBack = T, flushPipe = T
)
```

`rs1` 提供 VS-stage 虚拟地址语义，`rs2` 提供 VS-stage context 语义；当相应寄存器为 `x0` 时，`Fence.scala` 将其编码为 `sfence.bits.rs1/rs2` 的全范围选择条件。
...
| **源操作数** | 2 个整数源操作数：`rs1` 对应地址，`rs2` 对应 VS-stage context；实际物理值由重命名后的数据通路提供 |
...
| `s_wait` | 等待更老 SBuffer 内容排空 | `flushSb=1` | `func=hfence_v` 且 `sbIsEmpty=1` 时进入 `s_tlb` |
| `s_tlb` | TLB 刷新控制拍 | `sfence.valid=1`、`hv=1` | 下一拍返回 `s_idle` |
| 其他状态 | 本指令不使用 | `fencei=0` | 不适用于 `hfence.vvma` |

`hfence.vvma` 的 `sfence.bits.id` 不使用指令源寄存器中的 VMID，而由 MMU 侧当前 `hgatp.vmid` 参与匹配；`sfence.bits.addr` 锁存 `src(0)`。`rs1/rs2` 标志由指令立即数字段中对应寄存器编号是否为零决定。PageTableCache 的 `hfencev_valid` 分支只匹配 VS-stage（`onlyStage1`）翻译项。
...
| `sfence.bits.hv` | Fence → MMU | 1 | 标识 VS-stage HFENCE 请求 | 随 `sfence` 有效 |
| `sfence.bits.rs1/rs2` | Fence → MMU | 各 1 | 选择按地址、VS-stage context 或全范围失效 | 随请求携带 |
| `sfence.bits.addr` | Fence → MMU | XLEN | VS-stage 虚拟地址参数 | 输入握手时锁存 |
| `sfence.bits.id` | Fence → MMU | 由 ASID/VS-stage context 参数决定 | `hfence_v` 时为零扩展 VS-stage context | 输入握手时锁存 |
...
上图是源码约束的示意，不是当前版本实测波形。验证时应以 `TOP.clock` 正沿采样，先用 PC/指令字定位 `hfence.vvma`，进入后端后使用 `robIdx` 继续追踪；重点观察 `flushSb`、`sbIsEmpty`、`sfence.valid`、`hv`、`rs1/rs2`、`addr`、`id`、各级 TLB 失效以及 ROB 提交。
...
| 异常类型 | 检测位置 | 检测条件 | 处理方式 |
|---|---|---|---|
| 非法指令 | DecodeUnit/CSR 权限检查 | `illegalInst.hfenceVVMA` 置位，或当前特权状态不允许执行 | 记录非法指令异常，按 ROB 异常路径处理，不发出有效 TLB 刷新 |
...
| TLB/PTW 相关异常 | 后续访存/取指请求 | 刷新后重新翻译时发生权限或页表错误 | 由对应请求产生异常；`hfence.vvma` 本身不写回异常数据 |

HFENCE.VVMA 的执行权限由 CSR 状态参与判定；本地 `DecodeUnit.scala` 明确检查 `illegalInst.hfenceVVMA` 和 `virtualInst.hfence`，不能仅凭指令编码判断其一定可执行。
...
| HFENCE 正常执行 | MMU/TLB 局部失效 | 按 `rs1`、`rs2`、VS-stage context 和 VS-stage 地址匹配失效，不等于全流水线异常冲刷 | 后续翻译请求使用失效后的缓存状态 |
...
| VS-stage 页表更新 | 后续 guest 访存/取指 | 软件更新页表后使用 HFENCE.VVMA，使匹配的旧翻译项失效 |
| `rs1=x0` | 所有 VS-stage 虚拟页 | `sfence.bits.rs1=1`，PageTableCache 走不按单一地址限制的路径 |
| `rs2=x0` | 所有 VS-stage context | `sfence.bits.rs2=1`，不按单一 VS-stage context 限制 |
| `rs1!=x0` 或 `rs2!=x0` | 指定地址/VS-stage context | 由 PageTableCache 的匹配逻辑选择性失效 |
...
| `VMIDLength` | 由 MMU/Hypervisor 配置决定 | 影响当前 `hgatp.vmid` 的宽度和匹配逻辑 | [PageTableCache.scala][PTC] |
| `AsidLength` | 由 MMU 配置决定 | `sfence.bits.id` Bundle 宽度与 VS-stage context 对齐 | [Fence.scala][FENCE] |
...
| 模块 | 文件路径 | 关键内容 |
|---|---|---|
| HFENCE.VVMA 译码 | [backend/decode/DecodeUnit.scala][D] | `HFENCE.VVMA` → `FenceOpType.hfence_v` |
| 特权检查 | [backend/decode/DecodeUnit.scala][D] | `illegalInst.hfenceVVMA`、`virtualInst.hfence` |
...
| PageTableCache VS-stage 失效 | [cache/mmu/PageTableCache.scala][PTC] | `hfenceg_valid`、VS-stage context/VPN 匹配和 valid 清除 |
...
[PTC]: /nfs/home/wanghao/emuByYuan/stable-kmh-v2/src/main/scala/xiangshan/cache/mmu/PageTableCache.scala#L1157
