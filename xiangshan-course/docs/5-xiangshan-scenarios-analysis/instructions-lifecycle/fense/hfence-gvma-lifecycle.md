...
|---|---|
| **指令名称** | `hfence.gvma` |
| **编码格式** | `0110001_rs2[4:0]_rs1[4:0]_000_00000_1110011`，32 位；`funct7=0110001`、`funct3=000`、`rd=00000`、`opcode=1110011` |
...
`hfence.gvma` 不执行普通算术或访存，也不直接写回整数寄存器；它在 Fence FU 中等待 SBuffer 排空后，产生带有 `hg=1` 的 TLB 刷新请求。刷新范围由 `rs1`、`rs2` 的零值组合以及 VMID、G-stage 虚拟页号共同决定。
