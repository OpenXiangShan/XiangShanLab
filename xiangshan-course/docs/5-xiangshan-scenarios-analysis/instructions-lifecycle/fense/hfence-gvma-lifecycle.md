...
|---|---|
| **指令名称** | `hfence.gvma` |
| **编码格式** | `0001001_?????_?????_111_?????_1110011`；本地译码使用 `HFENCE_GVMA`，具体 `rs1`/`rs2` 携带虚拟地址与 VMID 选择信息 |
| **RISC-V 扩展** | Hypervisor 扩展 |
| **是否有压缩格式** | 否；没有对应的 C 扩展编码 |
| **指令分类** | 系统／虚拟内存管理／G-stage 地址翻译缓存失效 |
| **FuType** | `FuType.fence` |
| **FuOpType** | `FenceOpType.hfence_g` |
| **目标 FU** | Fence FU；经 `sfence` 接口通知 MMU/TLB |
| **分析日期** | 2026-09-06 |

本文以本地 `/nfs/home/wanghao/emuByYuan/stable-kmh-v2` 源码为准。`hfence.gvma` 不执行普通算术或访存，也不直接写回整数寄存器；它在 Fence FU 中等待 SBuffer 排空后，产生带有 `hg=1` 的 TLB 刷新请求。刷新范围由 `rs1`、`rs2` 的零值组合以及 VMID、G-stage 虚拟页号共同决定。
...
| 阶段 | 延迟 | 关键行为 | 代码位置 |
|---|---|---|---|
| ICache/MainPipe | 可变 | 普通取指获取包含 `hfence.gvma` 的指令块；该指令不是控制流指令 | [IFU.scala][IF]、[ICache.scala][IC] |
...
| 项目 | 内容 |
|---|---|
| **brTable 匹配** | `hfence.gvma` 不是分支、跳转、调用或返回指令，正常情况下不匹配控制流预测条目，`brType` 为非控制流类型 |
| **PreDecodeInfo 字段** | `valid=1`、`isRVC=0`、`brType=notCFI`，不产生 call/return 属性；具体字段由 [PreDecode.scala][PD] 定义 |
| **是否有专用检测逻辑** | 无前端专用 HFENCE 预译码逻辑；后端 `DecodeUnit` 识别 `HFENCE_GVMA` |
...
| 项目 | 内容 |
|---|---|
| **是否触发 remaskFault** | `hfence.gvma` 自身不满足控制流指令条件，因此不主动触发；同一窗口内其他指令仍可能触发 |
...
| 项目 | 内容 |
|---|---|
| **入队宽度** | 由 `PredictWidth=FetchWidth*(HasCExtension ? 2 : 1)` 等配置决定；不是 HFENCE 专属宽度 |
...
| **关键译码结果** | `fuType=FuType.fence`、`fuOpType=FenceOpType.hfence_g`、两个源操作数为 `SrcType.reg`、目标为 `SrcType.X`、`noSpec=true`、`blockBack=true`、`flushPipe=true` |
| **代码位置** | [DecodeUnit.scala][D] 490 行附近 |

```scala
HFENCE_GVMA -> XSDecode(
...
`rs1` 提供 G-stage 虚拟地址语义，`rs2` 提供 VMID 语义；当相应寄存器为 `x0` 时，`Fence.scala` 将其编码为 `sfence.bits.rs1/rs2` 的全范围选择条件。
...
| **源操作数** | 2 个整数源操作数：`rs1` 对应地址，`rs2` 对应 VMID；实际物理值由重命名后的数据通路提供 |
...
| **关键输出信号** | `flushSb`、`sfence.valid`、`sfence.bits.hg`、`sfence.bits.rs1/rs2`、`sfence.bits.addr`、`sfence.bits.id` |
| **代码位置** | [Fence.scala][FENCE] 26–100 行 |
...
| `s_wait` | 等待更老 SBuffer 内容排空 | `flushSb=1` | `func=hfence_g` 且 `sbIsEmpty=1` 时进入 `s_tlb` |
| `s_tlb` | TLB 刷新控制拍 | `sfence.valid=1`、`hg=1` | 下一拍返回 `s_idle` |
| 其他状态 | 本指令不使用 | `fencei=0` | 不适用于 `hfence.gvma` |

`sfence.bits.id` 对 `hfence_g` 使用 VMID：`Fence.scala` 从 `src(1)` 取低 `VmidLength` 位并零扩展；`sfence.bits.addr` 锁存 `src(0)`。`rs1/rs2` 标志由指令立即数字段中对应寄存器编号是否为零决定。
...
graph LR
  ...
  SB -->|sbIsEmpty| F
  F -->|sfence valid, hg, addr, id, rs1, rs2| MMU[PTW/TLB/MMU]
...
| `sfence.bits.hg` | Fence → MMU | 1 | 标识 G-stage HFENCE 请求 | 随 `sfence` 有效 |
| `sfence.bits.rs1/rs2` | Fence → MMU | 各 1 | 选择按地址、VMID 或全范围失效 | 随请求携带 |
| `sfence.bits.addr` | Fence → MMU | XLEN | G-stage 虚拟地址参数 | 输入握手时锁存 |
| `sfence.bits.id` | Fence → MMU | 由 ASID/VMID 参数决定 | `hfence_g` 时为零扩展 VMID | 输入握手时锁存 |
...
上图是源码约束的示意，不是当前版本实测波形。验证时应以 `TOP.clock` 正沿采样，先用 PC/指令字定位 `hfence.gvma`，进入后端后使用 `robIdx` 继续追踪；重点观察 `flushSb`、`sbIsEmpty`、`sfence.valid`、`hg`、`rs1/rs2`、`addr`、`id`、各级 TLB 失效以及 ROB 提交。
...
| 异常类型 | 检测位置 | 检测条件 | 处理方式 |
|---|---|---|---|
| 非法指令 | DecodeUnit/CSR 权限检查 | `illegalInst.hfenceGVMA` 置位，或当前特权状态不允许执行 | 记录非法指令异常，按 ROB 异常路径处理，不发出有效 TLB 刷新 |
| 虚拟指令异常 | DecodeUnit/CSR 权限检查 | `virtualInst.hfence` 置位，例如虚拟化特权状态不允许 | 记录虚拟指令异常，进入异常处理路径 |
| 取指页故障/访问故障 | IFU、ITLB、取指路径 | 包含该指令的取指请求翻译或权限失败 | 指令不能正常进入后端，按取指异常重定向 |
| TLB/PTW 相关异常 | 后续访存/取指请求 | 刷新后重新翻译时发生权限或页表错误 | 由对应请求产生异常；`hfence.gvma` 本身不写回异常数据 |

HFENCE.GVMA 的执行权限由 CSR 状态参与判定；本地 `DecodeUnit.scala` 明确检查 `illegalInst.hfenceGVMA` 和 `virtualInst.hfence`，不能仅凭指令编码判断其一定可执行。

### 5.2 冲刷与重定向

| 触发条件 | 类型 | 影响范围 | 恢复方式 |
|---|---|---|---|
| `flushPipe=true` 的 HFENCE 到达 ROB 顺序点 | 流水线刷新 | 清除年轻 uop，保持更老指令和 HFENCE 的顺序提交 | ROB 通过 `flushOut` 通知前端恢复 |
| HFENCE 正常执行 | MMU/TLB 局部失效 | 按 `rs1`、`rs2`、VMID 和 G-stage 地址匹配失效，不等于全流水线异常冲刷 | 后续翻译请求使用失效后的缓存状态 |
...
| G-stage 页表更新 | 后续 guest 访存/取指 | 软件更新页表后使用 HFENCE.GVMA，使匹配的旧翻译项失效 |
| `rs1=x0` | 所有 G-stage 虚拟页 | `sfence.bits.rs1=1`，PageTableCache 走不按单一地址限制的路径 |
| `rs2=x0` | 所有 VMID | `sfence.bits.rs2=1`，不按单一 VMID 限制 |
| `rs1!=x0` 或 `rs2!=x0` | 指定地址/VMID | 由 PageTableCache 的匹配逻辑选择性失效 |
| `hfence.vvma` / `sfence.vma` | VS-stage 或 guest 地址空间翻译 | 共享 Fence FU，但 `hv`/`hg` 标志不同，不能混淆刷新范围 |
| `hinval.gvma` | Hypervisor 无效化路径 | 也映射到 `hfence_g` 操作码族，但其译码控制字段可能不同，不能直接当作同一架构指令解释 |
...
HFENCE.GVMA 的架构效果依赖顺序点和参数匹配；文档不将“发出 `sfence.valid`”未经证据扩展为所有异步 MMU 结构已经同时完成刷新。
...
| TLB/PageTableCache 命中状态 | Cache/MMU 时序 | 按地址、VMID 和 G-stage 标记进行选择性失效；刷新后重新翻译 |
...
| HFENCE 请求带有明确 G-stage 标志 | `sfence.bits.hg := func === FenceOpType.hfence_g` | [Fence.scala][FENCE] |
| VMID 使用独立编码 | `hfence_g` 使用 `fenceVmid`，由源操作数低位零扩展 | [Fence.scala][FENCE] |
| 地址/VMID 全范围选择可编码 | `rs1/rs2` 由立即数字段判断是否为零 | [Fence.scala][FENCE] |
| TLB 按 G-stage 和参数匹配失效 | PageTableCache 使用 `hg`、VMID、G-stage 标记及 GVPN 进行选择性失效 | [PageTableCache.scala][PTC] |
...
| 指标 | 值 | 说明 |
|---|---|---|
| **执行吞吐** | 非流水化 Fence FU；不能按 DecodeWidth 推导连续 HFENCE 吞吐 | `FenceCfg.piped=false`，每条指令需经过等待和 TLB 控制状态 |
...
| `VmidLength` | 由 MMU/Hypervisor 配置决定 | `fenceVmid` 截取和零扩展宽度 | [Fence.scala][FENCE] |
| `AsidLength` | 由 MMU 配置决定 | `sfence.bits.id` Bundle 宽度与 VMID 对齐 | [Fence.scala][FENCE] |
...
| `FenceCfg.flushPipe` | `true` | HFENCE 顺序点提交时刷新年轻流水线 | [FuConfig.scala][F] |
| `FetchWidth` / `PredictWidth` | 由前端配置决定 | 影响 HFENCE 到达 IBuffer 的窗口宽度 | [Parameters.scala][P] |
...
| 模块 | 文件路径 | 关键内容 |
|---|---|---|
| HFENCE.GVMA 译码 | [backend/decode/DecodeUnit.scala][D] | `HFENCE_GVMA` → `FenceOpType.hfence_g` |
| 特权检查 | [backend/decode/DecodeUnit.scala][D] | `illegalInst.hfenceGVMA`、`virtualInst.hfence` |
...
| PageTableCache G-stage 失效 | [cache/mmu/PageTableCache.scala][PTC] | `hfenceg_valid`、VMID/GVPN 匹配和 valid 清除 |
...
[PTC]: /nfs/home/wanghao/emuByYuan/stable-kmh-v2/src/main/scala/xiangshan/cache/mmu/PageTableCache.scala#L1201
