...
| **FuType** | `FuType.ldu` |
| **FuOpType** | `LSUOpType.prefetch_r`，本地值 `b1001` |
...
本条指令表达未来读取数据的意图。**ROB 提交、DCache 接受缺失请求和缓存填充完成是三个不同事件**：hint 可以正常退休而未产生一次成功填充。与 `prefetch.i` 不同，本条不通过软件提示单槽转发至 Frontend；与普通 load 不同，本条没有架构目的寄存器，也不承诺通过重放最终取得数据。
...
> **前端流水线总延迟（无冲刷）：** F0→F3 连续推进时为三个阶段间隔；ICache miss、ITLB/PTW 等待与 IBuffer 排队另计，不把该数字当作数据预取延迟。
...
| **是否有专用检测逻辑** | 前端不执行读预取，后端 DecodeUnit 识别 ORI hint 编码 |
...
| **是否产生 wbRedirect** | 前端通用预测校验可能恢复，不是读预取产生控制流跳转 |
...
| **FSM 状态机** | 没有专属 PREFETCH.R 等待填充 FSM；LoadUnit/LoadPipe 使用阶段 valid，MissQueue 另管理被接受的事务 |
| **关键输出信号** | `dcache.req(cmd=M_PFR)`、`dcache.s1_kill/s2_kill`、`miss_req`、`ldout` |
...
| 阶段 | 条件/动作 | 后续行为 |
|---|---|---|
| LoadUnit S0 | 计算 `src(0)+SignExt(imm[11:0],VAddrBits)`，置 `prf/prf_rd` | 首发软件读预取正常查询 DTLB；不同于预翻译硬件预取和 `prf_i` |
| DCache 请求 | `cmd=M_PFR,instrtype=DCACHE_PREFETCH_SOURCE` | 读意图 cache 请求，不向 Frontend 发送 ifetchPrefetch |
...
MissQueue 区分 `isPrefetchRead` 与 `isPrefetchWrite`；已有同块事务下的晚到预取可能被忽略，不能声称所有 hint 都会分配一个新 MSHR 或都作为普通 load 合并。后续真正的 load 可以与已有预取事务按 MissQueue 条件交互。[MissQueue][MQ] 95–96、594–610、738–752 行
...
flowchart LR
  ...
  LD -->|虚拟地址| TLB[DTLB]
  TLB -->|翻译与异常| LD
  LD -->|M_PFR 与阶段控制| DC[DCache LoadPipe]
...
| `prf/prf_rd/prf_i` | LoadUnit 内 | 各 1 | 本条为 1/1/0 | 随来源和流水 |
| `dcache.req` | LoadUnit → LoadPipe | 请求 Bundle | `M_PFR`、地址、mask、prefetch 来源 | Decoupled |
| `tlbNoQuery` | LoadUnit 内 | 1 | 正常首发软件读预取不因自身类型置位 | 随流水 |
...
| 场景 | 周期数 | 条件 |
|---|---|---|
| **最佳** | 正常流水完成；目标命中无需新填充 | 基址已就绪、DTLB 命中、端口可用 |
...
| 交互场景 | 影响指令 | 行为 |
|---|---|---|
| load/add 生产基址 | prefetch.r | 等待 rs1；不是不依赖寄存器的硬件预取 |
| prefetch.r 后续 demand load | 同块读取 | 可能减少 miss，也可能太晚、被忽略或已替换；真实 load 仍需正常权限/数据检查 |
| 更老 store | 同地址或同块 | S1 SBuffer/LSQ 数据前递请求由 `!s1_prf` 门控，hint 不接收 store-forward 数据作为架构结果 |
| prefetch.w | 写意图 hint | 共用软件 hint 识别，命令分别是 M_PFR/M_PFW；读预取不等同写权限预取 |
