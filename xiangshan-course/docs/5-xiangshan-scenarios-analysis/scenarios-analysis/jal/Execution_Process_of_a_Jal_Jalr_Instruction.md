...
# ![figure-001-source-pc-offset](../../img/execution-process-of-a-jal-jalr-instruction/figure-001-source-pc-offset.png)

无条件跳转，源操作数为pc以及固定数据4，跳转目标为pc加上offset。

选择的追踪指令是如下这条：

![figure-002-select](../../img/execution-process-of-a-jal-jalr-instruction/figure-002-select.png)
...
![figure-003-decode-stage-result-waveform](../../img/execution-process-of-a-jal-jalr-instruction/figure-003-decode-stage-result-waveform.png)

在波形中可以确定这些信息，包括在valid信号有效的条件下，可以确定该指令的指令码为0x062000ef，便是上面我们追踪的指令。查看对于这条指令的译码结果：

包括于*srcType\_0以及*srcType\_1两个值都是0，

![figure-004-decode-stage-type-source](../../img/execution-process-of-a-jal-jalr-instruction/figure-004-decode-stage-type-source.png)

看代码发现了两个源操作数是来自于pc以及imm的，行为是正确的。

而不管是来自于pc还是来自imm的数，其具体的数值都是如下的结果：

![figure-005-decode-stage-pc-imm](../../img/execution-process-of-a-jal-jalr-instruction/figure-005-decode-stage-pc-imm.png)
...
![figure-006-decode-stage-fu-type](../../img/execution-process-of-a-jal-jalr-instruction/figure-006-decode-stage-fu-type.png)

从代码中可以确定，fuType的把热编码的第0位拉高即表明当前指令是一条jump相关的指令，波形行为正确。

![figure-007-decode-stage-fu-type](../../img/execution-process-of-a-jal-jalr-instruction/figure-007-decode-stage-fu-type.png)
...
![figure-008-decode-stage-pc](../../img/execution-process-of-a-jal-jalr-instruction/figure-008-decode-stage-pc.png)
