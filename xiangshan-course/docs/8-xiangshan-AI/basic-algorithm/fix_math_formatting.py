...
def fix_math_formatting(content):
    ...
    for line in lines:
        ...
        # 修复行内公式 $...$
        # 规则：
        # 1. $ 前面是字母、数字、中文、或 ) ] } 等，且后面不是空格、括号、特殊符号 -> 前面加空格
        # 2. $ 后面是字母、数字、中文，且前面不是空格、括号 -> 后面加空格

        # 在 $ 前面加空格的情况：
        # 前面是字母数字中文或右括号，且这个$不是紧跟左括号、反斜杠、或已经有空格
        line = re.sub(r'([\w一-鿿)\]}])\$(?=[^\s\(\[\{\\])', r'\1 $', line)

        # 在 $ 后面加空格的情况：
        # 后面是字母数字中文，且这个$前面不是右括号、反斜杠、或已经有空格
        line = re.sub(r'(?<=[^\s\)\]\}\\])\$([\w一-鿿])', r'$ \1', line)
