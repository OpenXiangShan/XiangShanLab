...
![`fence.i` 后旧指令可见性与潜在微架构状态残留](./fencei_issue_6132.png)

图左概括了本 Issue 的实际现象：代码被改写后，Fence FU 分别触发 ICache 失效和 ROB/FTQ redirect。由于两条控制路径不同步，旧 ICache line 产生的指令可能短暂到达 `cfVec`；随后 redirect 清除旧条目，处理器重新取指并退休修改后的函数，因此架构结果仍可以正确。

图右描述的是一种**潜在攻击原理**：如果旧指令包含由秘密值控制的 load、预取或分支，并且它在被 squash 前已经执行，就可能改变 Cache、TLB 或预测器状态。redirect 能撤销流水线中的指令，却通常不能回滚已经完成的 Cache 填充、替换状态或预测器更新。攻击者随后可以通过访问延迟推测这些残留状态，从而尝试恢复秘密信息。

这张图中的虚线泄露路径是风险模型，不是已证实的攻击链。图中 C4235-C4331 的 96 周期和 L1D/L2 填充来自 V2 对照；本次 V2 波形确认 11 条错误路径指令没有进入 LSQ/EXU/DCache，相关 Cache 填充也来自窗口前请求或已提交 store。V3 当前仅确认旧指令字在 `cfVec` 短暂可见，是否能够产生秘密相关且可稳定测量的状态仍需独立验证。
...
下表基于第 5 章的 V2 源码快照，用于确定修改边界；实现 V3 补丁前必须在 Issue 对应提交上重新核对接口和行号。
