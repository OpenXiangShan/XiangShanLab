...
# backend/regcache Analysis

## 1. Scope


- Analysis target: `backend/regcache`

- Source path: `/nfs/home/yuanmiaomiao/XiangShanLab/xiangshan-code/XiangShan_src/main/scala/xiangshan`

- Branch note: The user did not specify a branch; the skill defaults to Kunminghu v2. The local `XiangShan_src` is a source snapshot rather than a complete git checkout, so the branch name cannot be verified from the directory. This document therefore analyzes the local Kunminghu-style source snapshot.
...
- Course material: `11.PhysicalRegisterCache.md`, `8.BypassNetwork.md`

- Design Doc: No `XiangShan-Design-Doc` checkout was found locally, so no independent Design Doc verification was performed.


RegCache is a small physical-register result cache in the backend integer datapath. It does not hold architectural state or replace the physical register file; it caches recently written-back results so later consumers can read them through `DataSource.regcache` on a hit, reducing RF read-port pressure.
...
| RAW forwarding and bypassing | `8.BypassNetwork.md` | `BypassNetwork`, `DataSource` | `readForward/readBypass/readRegCache/readReg` | IssueQueue selects each source's `DataSource`; BypassNetwork uses `Mux1H` to select forward/bypass/regcache/RF/imm/zero | RegCache is a "short-term store of written-back values," not a same-cycle combinational bypass |

| Physical-register local cache | `11.PhysicalRegisterCache.md` | `RegCache`, `RegCacheDataModule` | `mem`, `v`, `readPorts`, `writePorts` | Execution results write the RegCache data array at the same time they write the RF | A hit is not guaranteed; a miss still falls back to the RF path |

| Tag lookup | `11.PhysicalRegisterCache.md` | `RegCacheTagTable`, `RegCacheTagModule` | `tag`, `valid`, `loadDependency`, `readPorts.valid/addr` | Dispatch searches the Int and Mem tag tables with `psrc`; a hit produces `useRegCache/regCacheIdx` | The tag tables and data arrays are separate, so invalidating a tag masks stale data |

| Structural conflicts | `8.BypassNetwork.md` | port-count params, asserts | `getExuRCReadSize`, `getExuRCWriteSize`, same-write asserts | RegCache port counts are generated from Exu parameters; multiple writes to one entry are asserted illegal | There is no dynamic arbitration; replacement and port mapping are expected to avoid conflicts |

| Replacement algorithm | `11.PhysicalRegisterCache.md` | `RegCacheAgeTimer`, `RegCacheAgeDetector` | `ageTimer`, `ageInfo`, `rowOnesSum` | The age timer generates an age-comparison matrix, and the detector selects the oldest, second-oldest, ... entry | This is not full-cache LRU; it uses 2-bit age plus an extra tie breaker and rank selection |

| Speculation and cancellation | `11.PhysicalRegisterCache.md` | `RegCacheTagTable`, Issue entry | `allocPregs`, `ldCancel`, `og0Cancel`, `replaceRC` | Tags may be written speculatively and are later cleared by allocation/load cancellation/replacement; IssueEntry also clears stale `rcIdx` | RegCache is not part of precise state; incorrect values are masked by invalidating the tag/`useRegCache` |

## 3. Design Intent vs Effective Code


- Course/design intent: RegCache caches recently written-back integer results to reduce contention for expensive RF read ports. RegCache is split into the IntRegCache and MemRegCache bank sets; TagTable maintains the `pdest -> rcIdx` mapping, and AgeTimer handles replacement.

- Effective source behavior: `RegCache` is instantiated only in the `DataPath` int-scheduler branch; `RegCacheTagTable` is instantiated in dispatch. The replacement index first travels from RegCache to IssueQueue, then returns to TagTable and later issue entries as wakeup `rcDest`.

- Unverified item: The XiangShan Design Doc description of RegCache was not independently checked locally; this document follows the effective Scala/Chisel source.
...
> This section was generated from the FSM, conflict, forward-progress, index/hash, cache-structure, exception/virtualization, and performance-bottleneck rules in `tools/verification-driver/skills`. Every expectation must be checked against the effective Chisel for the current `kunminghu-v2`.


| Verification ID | Risk / invariant | Directed stimulus | Expected observation | Checker / Coverage |
| --- | --- | --- | --- | --- |

| `C_MULTI_WRITE_SAME_ENTRY` | Multiple write ports overwrite the same RegCache entry in one cycle | Force two write ports to select the same slot | Trigger a source assertion or the sole legal priority; tag/data must not diverge; evidence [backend/regcache/RegCacheDataModule.scala:45-75](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/backend/regcache/RegCacheDataModule.scala#L45-L75) | Multi-write assertion; tag/data scoreboard |
| `REGCACHE_INVALID_READ` | Reading an invalid/stale slot produces a wrong operand | Read the old slot immediately after a tag miss, replacement cancellation, and load cancellation | Invalid reads are asserted/masked and cannot fabricate a hit; evidence [backend/regcache/RegCacheDataModule.scala:45-65](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/backend/regcache/RegCacheDataModule.scala#L45-L65) | Valid-bit checker; stale-slot reuse cover |
| `REGCACHE_TAG_DATA_ALIGN` | A pipelined write index sends the tag and data to different entries | Write different `pdest`/slots for three consecutive cycles and insert cancel/replace events | Tag writes, data writes, and the delayed index remain aligned; evidence [backend/regcache/RegCache.scala:65-120](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/backend/regcache/RegCache.scala#L65-L120) | Pipeline metadata scoreboard |
| `C_SAME_ENTRY_RW` | Same-cycle read/write/replace of one slot causes bypass or hit errors | Point read, write, and replace at the same entry simultaneously | Tag-hit, cancellation, and replacement priorities match the source; evidence [backend/regcache/RegCacheTagModule.scala:50-95](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/backend/regcache/RegCacheTagModule.scala#L50-L95) | Storage conflict checker; RAW/replace cross |
| `H_SAME_INDEX_DIFF_TAG` | Different `pdest` values contend for or duplicate a slot | Create different tags with the same low index and allow multiple slots to carry one tag | The TagTable hit vector, writes, and invalidations remain one-hot and explainable; evidence [backend/regcache/RegCacheTagTable.scala:55-105](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/backend/regcache/RegCacheTagTable.scala#L55-L105) | Tag uniqueness checker; multi-hit cover |
| `REGCACHE_AGE_ORDER` | The age matrix is not transitive or replacement entries are not unique | Read/write multiple entries, saturate timers, and compare across groups | Age-update priority and replacement one-hot assertions hold; evidence [backend/regcache/RegCacheAgeTimer.scala:51-98](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/backend/regcache/RegCacheAgeTimer.scala#L51-L98), [backend/regcache/AgeDetector.scala:63-70](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/backend/regcache/AgeDetector.scala#L63-L70) | Age-order checker; rank uniqueness cover |
| `RESOURCE_CONTENTION` | Active values are overwritten while all slots/read-write ports are busy | Fill all valid entries and continuously issue many reads and writes | Replacement selects only the oldest/invalid entry specified by the code; blocking does not cause an architectural error | Occupancy/arbiter checker; port-pressure cross |
| `PB_RECOVERY_THROUGHPUT` | Low hit rate or port conflicts keep the bypass network congested | Switch between cold and hot `pdest` phases while injecting sustained writeback | Misses/stale hits preserve correctness, and hit rate/port pressure recover | Performance checker; hit-rate/stall coverage |


### General Checking Principles


- The payload must remain stable while `valid && !ready`; only `fire` may advance a pointer or state, or perform one training update.

- Check flush/redirect/replay precedence according to the code; an error path must not commit, write a table, train a predictor, or expose an exception/data value.

- Once resources are full, verify that they can drain; repeated conflicts, retries, or redirects must not create deadlock/livelock, and starvation of low-priority old requests must be checked.

- Ring pointers must cover the wrap from the maximum value to zero; table indices must construct same-index/different-tag and same-cycle read/write conflict groups.
