...
# In-Depth Analysis of the Frontend Bim Branch Predictor
...
| Question | Answer |
| --- | --- |
| **Who** | The legacy `BIM` file describes a standalone bimodal direction predictor; the effective hardware is `TageBTable` inside `Tage.scala`. |
| **What** | It indexes 2-bit saturating counters with the PC and provides TAGE with a tagless base/alternate direction. |
| **How** | S0 computes the bank/index and reads the table; S1 selects the counter for each branch slot; update uses `WrBypass` to resolve same-address read-after-write and applies saturating increments/decrements. |
| **From what** | The lookup PC comes from the BPU's shared prediction request; training comes from the `update` generated by the FTQ after branch commit. |
| **To what** | The output feeds TAGE provider/alternate selection; changes are converted into redirects by the BPU's S2/S3 comparisons. |
...
This section lists the effective source entry points, key code skeletons, and behavioral explanations for `Bim / base table`, rather than retaining only filenames or line numbers.
...
| --- | --- | --- |
| `frontend/Bim.scala` | Entry point of the historical bimodal implementation; this baseline does not instantiate it directly in the effective Composer chain. | [frontend/Bim.scala](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/Bim.scala) |
| `frontend/Tage.scala` | TAGE base table as the currently effective base-direction table. | [frontend/Tage.scala#L143-L270](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/Tage.scala#L143-L270) |
| `Parameters.scala` | The effective predictor-chain parameters contain no standalone Bim component. | [Parameters.scala#L124-L143](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/Parameters.scala#L124-L143) |

<!-- ### 2.2. 核心代码骨架 -->
### 2.2. Core Code Skeleton
```scala
// 当前有效链路中，基础方向预测由 TAGE 内部 base table 承担
// In the currently effective path, base-direction prediction is handled by TAGE's internal base table.
...
This document must distinguish between the presence of Bim source code and the currently effective implementation. The effective Kunminghu-v2 predictor chain consists of FauFTB, Tage_SC, FTB, ITTAGE, and RAS; simple bimodal direction information is primarily represented by TAGE's base table, rather than by votes from a standalone Bim module.
## 3. Theory-to-Code Mapping

<!-- 本节把理论概念直接绑定到 `Bim / base table` 的源码对象、控制/数据状态和下游消费者。 -->
This section binds theoretical concepts directly to the source objects, control/data state, and downstream consumers of `Bim / base table`.
...
| --- | --- | --- | --- |
| Two-bit saturating counter | TAGE base counter | Provides a default direction at low cost. | TAGE provider/alternate selection |
| Aliasing conflict | base table index | Different PCs may map to the same base-direction entry. | Correction by TAGE tagged tables |
| Non-effective module boundary | Link configuration between `Bim.scala` and `Parameters.scala` | The presence of source code does not imply that an instance is effective. | Overview and BPU documentation |

<!-- ### 3.2. 阅读顺序 -->
### 3.2. Reading Order
<!-- 先按第 2 节定位源码对象，再顺着本表检查信号从哪里来、状态在哪里保存、何时更新、谁消费结果。若本篇只引用相邻模块拥有的状态，则以相邻 Frontend 分文档的源码分析为准。 -->
First locate the source objects using Section 2, then use this table to check where each signal originates, where state is stored, when it is updated, and who consumes the result. When this document refers only to state owned by an adjacent module, use the source analysis in the corresponding Frontend document as the authority.
<!-- ## 4. 论文原则和有效代码 -->
...
The standalone `Bim.scala` is entirely block-commented in this commit, so it has no effective FSM; the current `TageBTable` has an implicit lifecycle of reset scan, queryable operation, and update conflict. The classic theory comes from James E. Smith's *A Study of Branch Prediction Strategies*: a 2-bit counter crosses the strong/weak state boundary only after consecutive opposite outcomes, reducing a lasting flip caused by a single loop exit. The source is authoritative for XiangShan's specific banking, port-conflict, and bypass rules.

<!-- ### 4.2. 算法原理 -->
### 4.2. Algorithmic Principles
...
- A bimodal table has no FIFO-style underflow; tag misses do not apply because it is a tagless PC-indexed table, so the main risk is **aliasing**.
<!-- - SRAM reset 扫描期间 `ready` 被压低，避免读到未初始化计数器；单端口 update/read 冲突时响应 valid 被屏蔽。 -->
- During the SRAM reset scan, `ready` is deasserted to prevent reads of uninitialized counters; a single-port update/read conflict masks the response valid.
<!-- - `WrBypass` 容量有限，但它不是预测队列；旁路未命中时读取 SRAM 已提交值，不会数组越界。 -->
- `WrBypass` has finite capacity, but it is not a prediction queue; on a bypass miss, the committed SRAM value is read and no array out-of-bounds access occurs.
...
### 6.1. Control-Signal Breakdown: Who / From / To / How / Why
<!-- > 下表覆盖本文讲解中出现的查询、流水推进、选择、训练、替换和恢复控制。`为什么存在` 不以信号命名猜测，而以当前 `kunminghu-v2` 数据依赖、资源限制和恢复要求为依据。 -->
> The table below covers the lookup, pipeline advance, selection, training, replacement, and recovery control described in this document. The `Why it exists` column is based on the current `kunminghu-v2` data dependencies, resource limits, and recovery requirements, rather than inferred from signal names.
...
-->
| Control signal / state | Producer / origin | Consumer / destination | When and how it takes effect | Why it exists; consequence if absent | Source evidence |
| --- | --- | --- | --- | --- | --- |
| `io.req.valid / io.req.ready` | BPU/TAGE lookup pipeline | TageBTable SRAM read port | A PC-indexed read starts only when the handshake permits it; ready is deasserted during reset scanning or a write conflict. | The tagless base table may be accessed every cycle, so it needs an acceptance boundary; otherwise, a lookup could be treated as valid while SRAM is unreadable. | [frontend/Tage.scala:143-215](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/Tage.scala#L143-L215) |
| `io.req.bits.pc` | Shared BPU prediction request | Bank/index calculation | The PC's intra-block branch slot and table index jointly select a 2-bit counter. | Bim has no tag, so it can only use the PC to establish a stable, low-latency local-statistics baseline; without the PC, distinct static branches cannot be distinguished. | [frontend/Tage.scala:155-215](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/Tage.scala#L155-L215) |
| `doing_reset / reset_idx` | Internal TageBTable reset control | SRAM write port and ready | During reset, it writes weak initial values entry by entry and prevents normal lookups. | SRAM contents cannot be assumed to be deterministic after power-on; these signals prevent uninitialized counters from participating in prediction. | [frontend/Tage.scala:155-190](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/Tage.scala#L155-L190) |
| `io.update.valid` | FTQ commit training | Bim update pipeline | Only a valid committed update changes a counter; an update from a wrong path must not arrive. | It separates lookup from training, ensuring that long-lived state is modified only by resolved, attributable branch outcomes. | [frontend/Tage.scala:217-267](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/Tage.scala#L217-L267) |
| `io.update.bits.mask` | TAGE update selection | Counter write enable for each branch slot | Selects the counters to train in the current cycle by slot. | One fetch block may contain multiple conditional branches; the mask prevents one branch outcome from polluting the other slots in the same block. | [frontend/Tage.scala:217-267](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/Tage.scala#L217-L267) |
| `io.update.bits.taken` | Resolved branch outcome | `satUpdate` | It increments the counter for taken and decrements it for not-taken, saturating at both bounds. | Direction is the ground-truth training signal; saturation prevents an occasional loop exit from instantly reversing a strong prediction. | [frontend/Tage.scala:217-267](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/Tage.scala#L217-L267) |
| `update_bank_wen / update_bank_idx` | Update-address decoding | Single-port bank write port | Transforms a logical branch slot into a physical bank/index write mask. | A multi-slot layout and banked SRAM require explicit write selection; otherwise, the wrong physical counter may be written or multiple-write conflicts may occur. | [frontend/Tage.scala:217-267](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/Tage.scala#L217-L267) |
| `WrBypass.hit / data` | Pending-write bypass | Lookup-response counter | When a lookup and a recent write target the same address, it returns the newest counter rather than the stale SRAM value. | Write visibility in single-port/pipelined SRAM is delayed; the bypass removes stale read-after-write predictions. | [frontend/Tage.scala:224-241](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/Tage.scala#L224-L241) |
| `io.resp.valid` | SRAM response and conflict control | TAGE provider/alternate selection | Only a base-table read that truly completes may serve as the alternate. | Downstream logic must distinguish a not-taken prediction from no valid prediction in this cycle; otherwise, invalid would be misinterpreted as a direction value. | [frontend/Tage.scala:193-215](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/Tage.scala#L193-L215) |
...
Place the module back in the full Frontend path: it addresses at least one of prediction bandwidth, fetch correctness, memory-hierarchy latency, speculative recovery, or an upstream/downstream throughput mismatch.
...
Read the dynamic path as `valid -> ready -> fire -> register/state update -> consumer`, and examine normal operation, blocking, flush, redirect, replay, and post-recovery forward progress together.
...
Address, PC, folded history, tag, set/way, line offset, and FTQ offset must all be traced to source expressions; index conflicts, wraparound, and boundary-crossing cases are further developed in the algorithm and verification sections.
...
<!-- ### 10.1. 算法示例推演 -->
### 10.1. Algorithm Walkthrough Example
...
<!-- ### 10.2. 逐流水级算法 -->
### 10.2. Algorithm by Pipeline Stage
...
Record each table, stack, FIFO, MSHR, uncache entry, and pipeline register as observable `valid/full/empty/ready` state, and explain who writes it, who reads it, when it is cleared, and who receives backpressure when it is full or empty.
...
Stage descriptions use only source-level registers, valid/ready signals, and fire conditions; use F0/F1/F2/F3 for the Frontend and the actual Decode/Rename/Dispatch/Issue/Execute/Writeback/ROB boundaries for the Backend.
...
This predictor produces only prediction metadata; it does not directly treat a page crossing, Cache-Line crossing, or MMIO access as one atomic memory transaction. For a fetch block that crosses a boundary, the predictor chain first generates the block start PC, taken mask, target, and fall-through; IFU/ICache then independently performs address translation, permission checks, and memory-type classification for each address fragment. The BPU rules for comparing predictions and generating redirects in S1/S2/S3 are given in [frontend/BPU.scala:606-635](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/BPU.scala#L606-L635) and [frontend/BPU.scala:827-854](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/BPU.scala#L827-L854); therefore, when the second fragment incurs a page fault, line miss, or MMIO classification change, recovery targets the prediction history and FTQ context rather than silently merging the two fragments.
...
The smallest example is a remaining half of a 32-bit instruction at the end of a block: the first fragment may hit in the current Cache Line/page, while the second requires an independent request to the next line or page. IFU stores `lastHalf`, merges fragments across cycles, and clears it on flush ([frontend/IFU.scala:915-943](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/IFU.scala#L915-L943)). If the result from the second page or line changes the CFI position, target, or fall-through, BPU redirect comparison takes precedence over continuing with the stale prediction. For an MMIO/uncache address, the predictor can provide only a candidate PC; the actual access must enter IFU's MMIO FSM and wait for translation, PMP/PMA, and commit constraints, and a prediction hit cannot bypass side-effect control.
...
| Virtual page | Prediction block and history for the current page | Independent translation and permission result for the next page | Page/access/guest fault, flush, redirect |
...
Distinguish prediction errors, replay, page/access/guest faults, MMIO side effects, debug redirects, and architectural exceptions; identify the producer, priority, state to clear, recovery entry point, and commit visibility for each.
...
Enable control for the frontend branch predictors comes from `CustomCSRCtrlIO.bp_ctrl`, generated by the CSR module, rather than from a local private CSR in each predictor. The effective path is: `sbpctl` CSR field -> `io.status.custom.bp_ctrl` -> Backend `frontendCsrCtrl` -> XSCore `frontend.io.csrCtrl` -> Frontend `bpu.io.ctrl` -> each BPU sub-predictor's `io.enable`.
...
| `bp_ctrl.ubtbEnable` | `sbpctl.regOut.UBTB_ENABLE` | `ubtb.io.enable` | Enables or disables participation of S1 fast uBTB/MicroBtb lookup results in the prediction chain; the fall-through baseline remains when it is disabled. | [NewCSR.scala:1378](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/backend/fu/NewCSR/NewCSR.scala#L1378), [Bpu.scala:96](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/bpu/Bpu.scala#L96), [Bpu.scala:105](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/bpu/Bpu.scala#L105) |
...
| `bp_ctrl.tageEnable` | `sbpctl.regOut.TAGE_ENABLE` | `tage.io.enable` | Controls whether TAGE conditional-branch direction prediction is active; when disabled, a TAGE provider result cannot be used as the basis for a direction override. | [NewCSR.scala:1381](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/backend/fu/NewCSR/NewCSR.scala#L1381), [Bpu.scala:99](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/bpu/Bpu.scala#L99), [Bpu.scala:108](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/bpu/Bpu.scala#L108) |
...
| `bp_ctrl.scEnable` | `sbpctl.regOut.SC_ENABLE` | `sc.io.enable` | Controls whether the statistical corrector modifies the TAGE/base-direction result. | [NewCSR.scala:1382](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/backend/fu/NewCSR/NewCSR.scala#L1382), [Bpu.scala:100](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/bpu/Bpu.scala#L100), [Bpu.scala:109](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/bpu/Bpu.scala#L109) |
...
| `bp_ctrl.rasEnable` | `sbpctl.regOut.RAS_ENABLE` | `ras.io.enable` | Controls whether the return-address stack provides an override for RET/JALR return targets. | [NewCSR.scala:1384](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/backend/fu/NewCSR/NewCSR.scala#L1384), [Bpu.scala:102](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/bpu/Bpu.scala#L102), [Bpu.scala:111](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/bpu/Bpu.scala#L111) |
...
The `BpuCtrl` bundle explicitly defines seven Bool control bits: `ubtbEnable`, `abtbEnable`, `mbtbEnable`, `tageEnable`, `scEnable`, `ittageEnable`, and `rasEnable` ([Bundles.scala:179-189](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/bpu/Bundles.scala#L179-L189)). `CustomCSRCtrlIO` carries `bp_ctrl` as part of the CSR output ([Bundle.scala:586-596](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/Bundle.scala#L586-L596)). The Backend exposes `csrio.customCtrl` as `frontendCsrCtrl`, which XSCore then connects to the Frontend ([Backend.scala:526-527](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/backend/Backend.scala#L526-L527), [XSCore.scala:138](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/XSCore.scala#L138)). The Frontend first delays CSR control by `CsrCtrlPortDelay` and then sends `csrCtrl.bp_ctrl` to the BPU ([Frontend.scala:143-153](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/Frontend.scala#L143-L153)). The BPU adds a further two-cycle delay to meet timing, then distributes the controls to its sub-predictors ([Bpu.scala:89-111](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/bpu/Bpu.scala#L89-L111)).
...
| [docs/en/frontend/BPU/saturateCounter.md:1](https://github.com/OpenXiangShan/XiangShan-Design-Doc/blob/f8e258dc2d9c02c0616764856e1d18feedb91b81/docs/en/frontend/BPU/saturateCounter.md#L1) | A saturating counter stores direction state in a finite bit width and updates it during training. | [frontend/Bim.scala:79-123](https://github.com/OpenXiangShan/XiangShan/blob/52262f303fc06daf84cdab7011d59b7df65ce7e8/src/main/scala/xiangshan/frontend/Bim.scala#L79-L123) | Reads the prediction bit; updates the table entry when training is valid. | **Partially verified** | Design Doc describes the generic counter; this checkout's Bim integration is configuration-dependent. |
...
| [docs/en/frontend/BPU/mbtb.md:1](https://github.com/OpenXiangShan/XiangShan-Design-Doc/blob/f8e258dc2d9c02c0616764856e1d18feedb91b81/docs/en/frontend/BPU/mbtb.md#L1) | The BPU combines multiple predictors and sends the direction result to subsequent selection. | [frontend/BPU.scala:381-455](https://github.com/OpenXiangShan/XiangShan/blob/52262f303fc06daf84cdab7011d59b7df65ce7e8/src/main/scala/xiangshan/frontend/BPU.scala#L381-L455) | The BPU aggregates predictor responses and forms prediction metadata. | **Verified** | Exact Bim table wiring is hidden behind predictor configuration. |
...
| [docs/en/frontend/BPU/index.md:1](https://github.com/OpenXiangShan/XiangShan-Design-Doc/blob/f8e258dc2d9c02c0616764856e1d18feedb91b81/docs/en/frontend/BPU/index.md#L1) | A misprediction triggers redirect/recovery after later-stage validation. | [frontend/BPU.scala:606-635](https://github.com/OpenXiangShan/XiangShan/blob/52262f303fc06daf84cdab7011d59b7df65ce7e8/src/main/scala/xiangshan/frontend/BPU.scala#L606-L635) | Compares the prediction with actual/later-stage information and generates a redirect. | **Verified** | None |

<!-- ### 18.2. 当前真正生效的替代实现 -->
### 18.2. Currently Effective Replacement Implementation
...
- The Design Doc expresses intent and interface assumptions; the matrix below treats only material that can be located in effective Chisel for this source commit as implementation fact.
...
If the counter for a PC is `10` (weakly taken), one not-taken update changes it to `01` (weakly not-taken). On the next lookup of the same PC, TAGE can use this base direction if no longer-history tagged provider is available. If an update and lookup hit the same SRAM address, `WrBypass` provides the newest value and avoids reading a stale counter.
...
This section retains the original verification matrix and general checking principles; verification requirements remain governed by the effective source in the current `kunminghu-v2` baseline.

<!-- ### 21.1. 验证矩阵与通用判定原则 -->
### 21.1. Verification Matrix and General Checking Principles
<!-- > 本节依据 `tools/verification-driver/skills` 中的 FSM、冲突、前向进展、索引/哈希、缓存结构、异常/虚拟化和性能瓶颈规则生成。每个期望必须以当前 `kunminghu-v2` 有效 Chisel 为准。 -->
> This section is generated from the FSM, conflict, forward-progress, indexing/hashing, cache-structure, exception/virtualization, and performance-bottleneck rules in `tools/verification-driver/skills`. Every expectation must be based on effective Chisel in the current `kunminghu-v2` baseline.
...
| `F_RESET_IDLE` | No uninitialized prediction may be emitted during a reset scan. | Continuously provide a lookup PC before and after reset deassertion. | ready/response valid agree with reset state; the first valid counter/entry contains no stale value; evidence: [frontend/Tage.scala:155-215](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/Tage.scala#L155-L215). | FSM checker; reset/first-request cover |
...
| `H_SAME_INDEX_DIFF_TAG` | Bim is tagless, so aliasing may change only direction statistics. | Construct two PCs that map to the same `TageBTable` index. | Sharing a 2-bit counter is permitted, but the counter must saturate and must not pollute another bank/slot. | Index checker; counter-saturation cross |
...
| `C_SAME_ENTRY_RW` | Lookup and update target the same entry in the same cycle. | Make the lookup PC and committed update hit the same index/way. | Read-old/read-new/bypass/stall behavior agrees with the code; evidence: [frontend/Tage.scala:217-267](https://github.com/OpenXiangShan/XiangShan/blob/kunminghu-v2/src/main/scala/xiangshan/frontend/Tage.scala#L217-L267). | Storage-conflict checker; RAW-bypass cover |
...
| `C_MULTI_WRITE_SAME_ENTRY` | Multiple branch slots or update sources write the same entry. | Construct multiple valid update candidates in one cycle. | The write mask, priority, or illegal-condition assertion agrees with the code; a losing request must not be dropped without retry. | Multi-write checker; onehot/mask cover |
...
| `F_REQ_AND_FLUSH` | A wrong-path lookup/update competes with redirect. | Assert redirect/flush in the same cycle as lookup or update valid. | A wrong path must not train; pipeline metadata is cleared or restored to the correct FTQ entry. | Flush/replay checker; predictor-metadata scoreboard |
...
| `P_LIVELOCK_REPLAY_LOOP` | Persistent port conflict or update stall. | Continuously create lookup/write conflicts while releasing the port periodically. | Under fair conditions, lookup and update eventually complete without duplicate training. | Forward-progress checker; retry-exit cover |
...
| `PB_RECOVERY_THROUGHPUT` | Prediction bandwidth must not decrease permanently after a redirect under high load. | Inject a redirect after saturating lookup traffic, then restore a steady stream. | No stale prediction is observable, and the pipeline resumes sustained service within a bounded number of cycles. | Performance checker; recovery latency/throughput |

<!-- #### 21.1.1. 通用判定原则 -->
#### 21.1.1. General Checking Principles

<!-- - `valid && !ready` 期间 payload 必须稳定；只有 `fire` 才能推进指针、状态或训练一次。 -->
- While `valid && !ready`, the payload must remain stable; only `fire` may advance a pointer or state, or perform one training update.
<!-- - flush/redirect/replay 的胜负关系必须按代码优先级检查；错误路径不得提交、写表、训练预测器或暴露异常/数据。 -->
- The precedence among flush, redirect, and replay must be checked against the code; a wrong path must not commit, write a table, train a predictor, or expose an exception/data.
<!-- - 资源填满后必须验证可排空；重复冲突、retry 或 redirect 不得形成 deadlock/livelock，并检查低优先级旧请求是否饥饿。 -->
- A full resource must be shown to drain; repeated conflict, retry, or redirect must not form a deadlock/livelock, and old low-priority requests must be checked for starvation.
