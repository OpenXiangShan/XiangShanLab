...
For backend top-level or full-chain analysis, include a dedicated Mermaid stage graph after the module graph. Use the actual stage names and boundaries from source code: normally Decode, Rename, Dispatch, Issue, Execute, Writeback, and Commit, but preserve `s0/s1`, `B0/B1`, or other implementation-specific names when present. Label only bundled payload/control groups between stages and keep each module pair to at most three edges. Draw redirect, flush, wakeup, replay, and commit feedback as a separate compact path; split the graph into decode/rename, dispatch/issue/execute, and writeback/commit subgraphs if the edge budget would be exceeded.
