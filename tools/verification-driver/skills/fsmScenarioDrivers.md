...
| `F_ENQ_DEQ_FLUSH` | Queue FSM conflict | Enqueue, dequeue, and flush same cycle | Pointer/status update priority follows code | Killed entry not visible; survivor preserved | Occupancy/FSM checker |
...
- Flush, redirect, replay, exception, interrupt/debug, debug entry/resume, reset, and context switch are tested while the FSM is not idle when reachable. Debug entry/resume FSMs also use `skills/debugEventDrivers.md`.
